...
class CatsMultitool:
    def __init__(self, root):
        ...
        self.status = tk.Label(root, text="AC HOLDINGS MULTITOOL v0.1.13 | PROTOCOL ZERO | NYAH~", 
        ...
        def run():
            try:
                if platform.system() == "Darwin":
                    ...
                    result = subprocess.check_output("nmcli dev wifi list 2>/dev/null || iwlist scan 2>/dev/null", shell=True, timeout=10).decode()
    ...
    def build_burp_tab(self, frame):
        tk.Label(frame, text="=== HTTP REPEATER ===", bg="#0A0A0A", fg="#00FFAA", font=("Courier", 14, "bold")).pack(pady=10)
        tk.Label(frame, text="URL:", bg="#0A0A0A", fg="#00FFAA").pack(anchor="w", padx=10)
        self.url_entry = tk.Entry(frame, width=80, bg="#1A1A1A", fg="#00FFAA", insertbackground="#00FFAA")
        self.url_entry.pack(padx=10, fill="x", pady=2)
        self.url_entry.insert(0, "https://example.com")

        method_frame = tk.Frame(frame, bg="#0A0A0A")
        method_frame.pack(fill="x", padx=10, pady=5)
        tk.Label(method_frame, text="Method:", bg="#0A0A0A", fg="#00FFAA").pack(side="left")
        self.method_var = tk.StringVar(value="GET")
        for m in ["GET", "POST", "PUT", "DELETE"]:
            tk.Radiobutton(method_frame, text=m, variable=self.method_var, value=m, bg="#0A0A0A", fg="#00FFAA", selectcolor="#003322").pack(side="left", padx=8)

        tk.Label(frame, text="Body:", bg="#0A0A0A", fg="#00FFAA").pack(anchor="w", padx=10, pady=(8,0))
        self.body_text = scrolledtext.ScrolledText(frame, height=6, bg="#1A1A1A", fg="#00FFAA")
        self.body_text.pack(padx=10, fill="x", pady=5)

        tk.Button(frame, text="LAUNCH REQUEST", bg="#002211", fg="#00FFAA", font=("Courier", 12, "bold"),
                  command=self.send_http, height=2, width=30).pack(pady=12)

        self.http_output = scrolledtext.ScrolledText(frame, height=12, bg="#000000", fg="#00FFAA", font=("Courier", 10))
